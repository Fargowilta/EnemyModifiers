    ...
    public class Possessed : Modifier
    {
        ...
        private static void PossessedAI(NPC npc)
        {
            ...
            for (int num308 = num284; num308 < num284 + num285; num308++)
            {
                ...
                if (!Main.tile[num283, num308].IsActuated && Main.tileSolid[Main.tile[num283, num308].TileType] ||
                    Main.tile[num283, num308].LiquidType > 0)
                {
                    if (num308 <= num284 + 1)
                    {
                    }
            ...
            if (Main.player[npc.target].npcTypeNoAggro[npc.type])
            {
                ...
                for (int num309 = num284; num309 < num284 + num285 - 2; num309++)
                {
                    ...
                    if (!Main.tile[num283, num309].IsActuated && Main.tileSolid[Main.tile[num283, num309].TileType] ||
                        Main.tile[num283, num309].LiquidType > 0)
                    {
                        flag25 = true;
                        break;
                    }
