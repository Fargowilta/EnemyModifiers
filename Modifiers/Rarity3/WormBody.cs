        ...
        private void WormAI(NPC npc)
        {
            ...
            if (!flag2)
            {
                for (int num33 = num29; num33 < num30; num33++)
                {
                    ...
                    {
                        if (Main.tile[num33, num34] != null && (!Main.tile[num33, num34].IsActuated &&
                                                                (Main.tileSolid[Main.tile[num33, num34].TileType] ||
                                                                 Main.tileSolidTop
                                                                     [Main.tile[num33, num34].TileType] &&
                                                                 Main.tile[num33, num34].TileFrameY == 0) ||
                                                                Main.tile[num33, num34].LiquidType > 64))
                        {
                            Vector2 vector;
                            vector.X = num33 * 16;
                            vector.Y = num34 * 16;
                            ...
                            {
                                flag2 = true;
                                if (Main.rand.Next(100) == 0 && !Main.tile[num33, num34].IsActuated)
                                {
            ...
            if (npc.localAI[1] > 0f && npc.localAI[1] < Main.npc.Length)
            {
                try
                {
                    vector2 = new Vector2(npc.position.X + npc.width * 0.5f,
                        npc.position.Y + npc.height * 0.5f);
                    num40 = Main.npc[(int) npc.localAI[1]].position.X +
                        Main.npc[(int) npc.localAI[1]].width / 2 - vector2.X;
                    num41 = Main.npc[(int) npc.localAI[1]].position.Y +
                        Main.npc[(int) npc.localAI[1]].height / 2 - vector2.Y;
                }
                catch
                {
                }
            ...
            else
            {
                ...
                else
                {
                    if (npc.soundDelay == 0)
                    {
                        ...
                        //SoundEngine.PlaySound(SoundID. 15, npc.Center);
                    }
                    ...
                    if (flag4)
                    {
                        ...
                        if (flag5)
                        {
                            if (Main.netMode != 1 && npc.position.Y / 16f >
                                (Main.rockLayer + Main.maxTilesY) / 2.0)
                            {
                                npc.active = false;
